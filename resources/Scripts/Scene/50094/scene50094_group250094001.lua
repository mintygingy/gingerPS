local base_info = {group_id = 250094001}
monsters = {
  {
    config_id = 1005,
    monster_id = 24030301,
...
function action_EVENT_SELECT_OPTION_1002(context, evt)
  ...
  if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 250094001, 1001, 175) then
    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : del_work_options_by_group_configId")
    return -1
  end
  ScriptLib.AddExtraGroupSuite(context, 250094001, 2)
  return 0
end
...
function action_EVENT_ANY_MONSTER_LIVE_1010(context, evt)
  if 0 ~= ScriptLib.ActiveChallenge(context, 1, 166, 180, 250094001, 1, 0) then
...
function action_EVENT_CHALLENGE_FAIL_1014(context, evt)
  if 0 ~= ScriptLib.RefreshGroup(context, {group_id = 250094001, suite = 1}) then
