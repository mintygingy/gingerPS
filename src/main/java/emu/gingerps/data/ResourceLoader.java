package emu.gingerps.data;

import com.google.gson.annotations.SerializedName;
import emu.gingerps.gingerps;
import emu.gingerps.data.binout.*;
import emu.gingerps.data.binout.AbilityModifier.AbilityModifierAction;
import emu.gingerps.data.common.PointData;
import emu.gingerps.game.managers.blossom.BlossomConfig;
import emu.gingerps.game.quest.QuestEncryptionKey;
import emu.gingerps.game.world.SpawnDataEntry;
import emu.gingerps.game.world.SpawnDataEntry.GridBlockId;
import emu.gingerps.game.world.SpawnDataEntry.SpawnGroupEntry;
import emu.gingerps.scripts.SceneIndexManager;
import emu.gingerps.utils.FileUtils;
import emu.gingerps.utils.JsonUtils;
import emu.gingerps.utils.TsvUtils;
...
import static emu.gingerps.utils.FileUtils.getDataPath;
import static emu.gingerps.utils.FileUtils.getResourcePath;
...
public class ResourceLoader {
    ...
    protected static List<Set<Class<?>>> getResourceDefClassesPrioritySets() {
        ...
        gingerps.getLogger().debug("Priorities are "+priorities);
    ...
    public static void loadAll() {
        if (loadedAll) return;
        gingerps.getLogger().info(translate("messages.status.resources.loading"));
        ...
        gingerps.getLogger().info(translate("messages.status.resources.finish"));
        loadedAll = true;
    }
    ...
    public static void loadResources(boolean doReload) {
        ...
        errors.forEach(pair -> gingerps.getLogger().error("Error loading resource file: " + pair.left(), pair.right()));
        long endTime = System.nanoTime();
        long ns = (endTime - startTime);  //divide by 1000000 to get milliseconds.
        gingerps.getLogger().debug("Loading resources took "+ns+"ns == "+ns/1000000+"ms");
    }
    ...
    private static void loadScenePoints() {
        ...
        try {
            ...
            gingerps.getLogger().error("Scene point files cannot be found, you cannot use teleport waypoints!");
            return;
        }
    }
    ...
    private static void loadAbilityEmbryos() {
        ...
        if (embryoList == null) {
            ...
            try {
                Files.newDirectoryStream(getResourcePath("BinOutput/Avatar/"), "ConfigAvatar_*.json").forEach(path -> {
                    ...
                    try {
                        ...
                        gingerps.getLogger().error("Error loading player ability embryos:", e);
                        return;
                    }
                ...
                gingerps.getLogger().error("Error loading ability embryos: no files found");
                return;
            }
            ...
            try {
                ...
                gingerps.getLogger().error("Error loading player abilities:", e);
            }
        }

        if (embryoList == null || embryoList.isEmpty()) {
            gingerps.getLogger().error("No embryos loaded!");
            return;
        }
    ...
    private static void loadAbilityModifiers() {
        ...
        try (Stream<Path> paths = Files.walk(getResourcePath("BinOutput/Ability/Temp/"))) {
            ...
            gingerps.getLogger().error("Error loading ability modifiers: ", e);
            return;
        }
    ...
    private static void loadAbilityModifiers(Path path) {
        try {
            ...
            gingerps.getLogger().error("Error loading ability modifiers from path " + path.toString() + ": ", e);
            return;
        }
    }
    ...
    private static void loadSpawnData() {
        ...
        if (spawnEntryMap.isEmpty()) {
            gingerps.getLogger().error("No spawn data loaded!");
            return;
        }
    ...
    private static void loadOpenConfig() {
        ...
        if (list == null) {
            ...
            for (String folderName : folderNames) {
                try {
                    ...
                    gingerps.getLogger().error("Error loading open config: no files found in " + folderName);
                    return;
                }
            }
        ...
        if (list == null || list.isEmpty()) {
            gingerps.getLogger().error("No openconfig entries loaded!");
            return;
        }
    ...
    private static void loadQuests() {
        try {
            ...
            gingerps.getLogger().error("Quest data missing");
            return;
        }
        ...
        try {
            ...
            gingerps.getLogger().debug("Loaded {} quest keys.", questEncryptionMap.size());
        } catch (Exception e) {
            gingerps.getLogger().error("Unable to load quest keys.", e);
        }

        gingerps.getLogger().debug("Loaded " + GameData.getMainQuestDataMap().size() + " MainQuestDatas.");
    }
    ...
    public static void loadScriptSceneData() {
        try {
            ...
            gingerps.getLogger().debug("Loaded " + GameData.getScriptSceneDataMap().size() + " ScriptSceneDatas.");
        } catch (IOException e) {
            gingerps.getLogger().debug("ScriptSceneData folder missing or empty.");
            return;
        }
    }
    ...
    private static void loadHomeworldDefaultSaveData() {
        ...
        try {
            ...
            gingerps.getLogger().debug("Loaded " + GameData.getHomeworldDefaultSaveData().size() + " HomeworldDefaultSaveDatas.");
        } catch (IOException e) {
            gingerps.getLogger().error("Failed to load HomeworldDefaultSave folder.");
        }
    }
    ...
    private static void loadNpcBornData() {
        try {
            ...
            gingerps.getLogger().debug("Loaded " + GameData.getSceneNpcBornData().size() + " SceneNpcBornDatas.");
        } catch (IOException e) {
            gingerps.getLogger().error("Failed to load SceneNpcBorn folder.");
        }
    }
        ...
        try {
            Files.newDirectoryStream(getResourcePath("BinOutput/Gadget/"), "*.json").forEach(path -> {
                try {
                    ...
                    gingerps.getLogger().error("failed to load ConfigGadget entries for " + path.toString(), e);
                    return;
                }
            });

            gingerps.getLogger().debug("Loaded {} ConfigGadget entries.", GameData.getGadgetConfigData().size());
        } catch (IOException e) {
            gingerps.getLogger().error("Failed to load ConfigGadget folder.");
        }
    }
    ...
    private static void loadBlossomResources() {
        try {
            ...
            gingerps.getLogger().debug("Loaded BlossomConfig.");
        } catch (IOException e) {
            gingerps.getLogger().warn("Failed to load BlossomConfig.");
        }
    }
