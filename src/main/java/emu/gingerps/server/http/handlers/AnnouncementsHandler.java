package emu.gingerps.server.http.handlers;

import emu.gingerps.gingerps;
import emu.gingerps.data.DataLoader;
import emu.gingerps.server.http.objects.HttpJsonResponse;
...
public final class AnnouncementsHandler implements Router {
        ...
        if (Objects.equals(ctx.endpointHandlerPath(), "/common/hk4e_global/announcement/api/getAnnContent")) {
            try {
                ...
                if (e.getClass() == IOException.class) {
                    gingerps.getLogger().info("Unable to read file 'GameAnnouncementList.json'. \n" + e);
                }
            }
    ...
    private static void getPageResources(Context ctx) {
        ...
        try (InputStream filestream = DataLoader.load(stringJoiner.toString())) {
            ...
            gingerps.getLogger().warn("File does not exist: " + ctx.path());
            ctx.status(404);
        }
    }
}
