...
import emu.gingerps.net.proto.QueryCurrRegionHttpRspOuterClass.QueryCurrRegionHttpRsp;
import emu.gingerps.net.proto.RegionInfoOuterClass.RegionInfo;
import emu.gingerps.net.proto.RegionSimpleInfoOuterClass.RegionSimpleInfo;
import emu.gingerps.server.event.dispatch.QueryAllRegionsEvent;
import emu.gingerps.server.event.dispatch.QueryCurrentRegionEvent;
import emu.gingerps.server.http.Router;
import emu.gingerps.server.http.objects.QueryCurRegionRspJson;
...
import static emu.gingerps.net.proto.QueryRegionListHttpRspOuterClass.QueryRegionListHttpRsp;
...
public final class RegionHandler implements Router {
    ...
    public RegionHandler() {
        try { // Read & initialize region data.
            ...
            gingerps.getLogger().error("Failed to initialize region data.", exception);
        }
    }
    ...
    private void initialize() {
        ...
        if (SERVER.runMode != ServerRunMode.HYBRID && configuredRegions.size() == 0) {
            gingerps.getLogger().error("[Dispatch] There are no game servers available. Exiting due to unplayable state.");
            System.exit(1);
        ...
        configuredRegions.forEach(region -> {
            if (usedNames.contains(region.Name)) {
                gingerps.getLogger().error("Region name already in use.");
                return;
            }
    ...
    private static void queryRegionList(Context ctx) {
        ...
        // Log to console.
        gingerps.getLogger().info(String.format("[Dispatch] Client %s request: query_region_list", ctx.ip()));
    }
    ...
    private static void queryCurrentRegion(Context ctx) {
        ...
        if (versionMajor >= 3 || (versionMajor == 2 && versionMinor == 7 && versionFix >= 50) || (versionMajor == 2 && versionMinor == 8)) {
            try {
                ...
                gingerps.getLogger().error("An error occurred while handling query_cur_region.", e);
            }
        }
        else {
            // Invoke event.
        ...
        // Log to console.
        gingerps.getLogger().info(String.format("Client %s request: query_cur_region/%s", ctx.ip(), regionName));
    }
