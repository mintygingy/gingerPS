package emu.gingerps.server.http;
...
public final class HttpServer {
    ...
    private static Server createServer() {
        ...
        if (HTTP_ENCRYPTION.useEncryption) {
            ...
            if (!keystoreFile.exists()) {
                ...
                gingerps.getLogger().warn(translate("messages.dispatch.keystore.no_keystore_error"));
            } else try {
                sslContextFactory.setKeyStorePath(keystoreFile.getPath());
                sslContextFactory.setKeyStorePassword(HTTP_ENCRYPTION.keystorePassword);
            } catch (Exception ignored) {
                gingerps.getLogger().warn(translate("messages.dispatch.keystore.password_error"));

                try {
                    sslContextFactory.setKeyStorePath(keystoreFile.getPath());
                    sslContextFactory.setKeyStorePassword("123456");

                    gingerps.getLogger().warn(translate("messages.dispatch.keystore.default_password"));
                } catch (Exception exception) {
                    gingerps.getLogger().warn(translate("messages.dispatch.keystore.general_error"), exception);
                }
            } finally {
    ...
    public HttpServer addRouter(Class<? extends Router> router, Object... args) {
        ...
        try { // Create a router instance & apply routes.
            ...
            gingerps.getLogger().warn(translate("messages.dispatch.router_error"), exception);
        } return this;
    }
    ...
    public void start() throws UnsupportedEncodingException {
        ...
        gingerps.getLogger().info(translate("messages.dispatch.address_bind", HTTP_INFO.accessAddress, this.javalin.port()));
    }
        ...
        @Override public void applyRoutes(Javalin javalin) {
            javalin.error(404, ctx -> {
                ...
                if (DISPATCH_INFO.logRequests == ServerDebugMode.MISSING)
                    gingerps.getLogger().info(translate("messages.dispatch.unhandled_request_error", ctx.method(), ctx.url()));
                // Send file
