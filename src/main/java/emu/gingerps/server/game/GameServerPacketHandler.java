...
import emu.gingerps.server.event.game.ReceivePacketEvent;
...
public class GameServerPacketHandler {
    ...
    public void registerHandlers(Class<? extends PacketHandler> handlerClass) {
        Reflections reflections = new Reflections("emu.gingerps.server.packet");
        ...
        // Debug
        gingerps.getLogger().debug("Registered " + this.handlers.size() + " " + handlerClass.getSimpleName() + "s");
    }
    ...
    public void handle(GameSession session, int opcode, byte[] header, byte[] payload) {
        ...
        if (GAME_INFO.logPackets == ServerDebugMode.MISSING || GAME_INFO.logPackets == ServerDebugMode.ALL) {
            gingerps.getLogger().info("Unhandled packet (" + opcode + "): " + emu.gingerps.net.packet.PacketOpcodesUtils.getOpcodeName(opcode));
        }
    }
}
