...
import emu.gingerps.net.packet.PacketOpcodesUtils;
import emu.gingerps.server.event.game.SendPacketEvent;
...
public class GameSession implements GameSessionManager.KcpChannel {
    ...
    public void logPacket(String sendOrRecv, int opcode, byte[] payload) {
        gingerps.getLogger().info(sendOrRecv + ": " + PacketOpcodesUtils.getOpcodeName(opcode) + " (" + opcode + ")");
    ...
    public void send(BasePacket packet) {
        ...
        if (packet.getOpcode() <= 0) {
            gingerps.getLogger().warn("Tried to send packet with missing cmd id!");
            return;
        }
    ...
    public void onConnected(GameSessionManager.KcpTunnel tunnel) {
        this.tunnel = tunnel;
        gingerps.getLogger().info(translate("messages.game.connect", this.getAddress().toString()));
    }
    ...
    public void handleReceive(byte[] bytes) {
        ...
        try {
            ...
            while (packet.readableBytes() > 0) {
                ...
                if (const1 != 17767) {
                    if (allDebug) {
                        gingerps.getLogger().error("Bad Data Package Received: got {} ,expect 17767", const1);
                    }
                    return; // Bad packet
                }
                // Data
                ...
                if (const2 != -30293) {
                    if (allDebug) {
                        gingerps.getLogger().error("Bad Data Package Received: got {} ,expect -30293", const2);
                    }
                    return; // Bad packet
                }
    ...
    public void handleClose() {
        ...
        gingerps.getLogger().info(translate("messages.game.disconnect", this.getAddress().toString()));
        ...
        try {
            ...
            gingerps.getLogger().warn("closing {} error", getAddress().getAddress().getHostAddress());
        }
        tunnel = null;
    }
