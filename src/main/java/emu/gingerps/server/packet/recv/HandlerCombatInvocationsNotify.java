...
import emu.gingerps.net.proto.EvtAnimatorParameterInfoOuterClass.EvtAnimatorParameterInfo;
...
import emu.gingerps.net.proto.PlayerDieTypeOuterClass;
import emu.gingerps.server.event.entity.EntityMoveEvent;
...
public class HandlerCombatInvocationsNotify extends PacketHandler {
    ...
    private void handleFallOnGround(GameSession session, GameEntity entity, MotionState motionState) {
        ...
        gingerps.getLogger().trace("MOTION_FALL_ON_GROUND received after " + actualDelay + "/" + maxDelay + "ms." + (actualDelay > maxDelay ? " Discard" : ""));
        ...
        if (damageFactor > 0) {
            gingerps.getLogger().debug(currentHP + "/" + maxHP + "\tLandingSpeed: " + cachedLandingSpeed +
                    "\tDamageFactor: " + damageFactor + "\tDamage: " + damage + "\tNewHP: " + newHP);
        } else {
            gingerps.getLogger().trace(currentHP + "/" + maxHP + "\tLandingSpeed: 0\tNo damage");
        }
