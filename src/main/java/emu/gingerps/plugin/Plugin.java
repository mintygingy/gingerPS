...
public abstract class Plugin {
    ...
    private void initializePlugin(PluginIdentifier identifier, URLClassLoader classLoader) {
        if (this.identifier != null) {
            gingerps.getLogger().warn(this.identifier.name + " had a reinitialization attempt.");
            return;
        }
        ...
        if (!this.dataFolder.exists() && !this.dataFolder.mkdirs()) {
            gingerps.getLogger().warn("Failed to create plugin data folder for " + this.identifier.name);
            return;
        }
    }
