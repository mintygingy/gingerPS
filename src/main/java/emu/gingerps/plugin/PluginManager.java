package emu.gingerps.plugin;

import emu.gingerps.gingerps;
import emu.gingerps.server.event.*;
...
public final class PluginManager {
    ...
    private void loadPlugins() {
        ...
        if (!pluginsDir.exists() && !pluginsDir.mkdirs()) {
            gingerps.getLogger().error(translate("plugin.directory_failed", pluginsDir.getAbsolutePath()));
            return;
        }
        ...
        plugins.forEach(plugin -> {
            try {
                ...
                gingerps.getLogger().warn(translate("plugin.unable_to_load"), exception);
            }
        });
            ...
            try {
                ...
                try (URLClassLoader loader = new URLClassLoader(new URL[]{url})) {
                    ...
                    if (!pluginConfig.validate()) {
                        gingerps.getLogger().warn(translate("plugin.invalid_config", plugin.getName()));
                        return;
                    }
                    ...
                    gingerps.getLogger().warn(translate("plugin.invalid_main_class", plugin.getName()));
                } catch (FileNotFoundException ignored) {
                    gingerps.getLogger().warn(translate("plugin.missing_config", plugin.getName()));
                }
            } catch (Exception exception) {
                gingerps.getLogger().error(translate("plugin.failed_to_load_plugin", plugin.getName()), exception);
            }
        }
        ...
        while (!dependencies.isEmpty()) {
            ...
            if (depth >= maxDepth) {
                gingerps.getLogger().error(translate("plugin.failed_to_load_dependencies"));
                break;
            }
            ...
            try {
                ...
                gingerps.getLogger().error(translate("plugin.failed_to_load"), exception); depth++;
            }
        }
    }
    ...
    private void loadPlugin(Plugin plugin, PluginIdentifier identifier, URLClassLoader classLoader) {
        gingerps.getLogger().info(translate("plugin.loading_plugin", identifier.name));
        ...
        try {
            ...
            gingerps.getLogger().warn(translate("plugin.failed_add_id", identifier.name));
        }
        ...
        try {
            ...
            gingerps.getLogger().error(translate("plugin.failed_to_load_plugin", identifier.name), exception);
        }
    }
    ...
    public void enablePlugins() {
        this.plugins.forEach((name, plugin) -> {
            gingerps.getLogger().info(translate("plugin.enabling_plugin", name));
            try {
                plugin.onEnable();
            } catch (Throwable exception) {
                gingerps.getLogger().error(translate("plugin.enabling_failed", name), exception);
            }
        });
    }
    ...
    public void disablePlugins() {
        this.plugins.forEach((name, plugin) -> {
            gingerps.getLogger().info(translate("plugin.disabling_plugin", name));
    ...
    public void enablePlugin(Plugin plugin) {
        try {
            ...
            gingerps.getLogger().error(translate("plugin.enabling_failed", plugin.getName()), exception);
        }
    }
    ...
    public void disablePlugin(Plugin plugin) {
        try {
            ...
            gingerps.getLogger().error(translate("plugin.disabling_failed", plugin.getName()), exception);
        }
