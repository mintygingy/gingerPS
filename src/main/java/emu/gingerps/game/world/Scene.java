...
import emu.gingerps.data.excels.*;
...
import emu.gingerps.game.managers.blossom.BlossomManager;
...
import emu.gingerps.net.proto.SelectWorktopOptionReqOuterClass;
...
import emu.gingerps.scripts.data.SceneBlock;
...
public class Scene {
    ...
    public void checkNpcGroup() {
        ...
        if (toUnload.size() > 0) {
            ...
            gingerps.getLogger().debug("Unload NPC Group {}", toUnload);
        }
    ...
    public List<SceneBlock> getPlayerActiveBlocks(Player player) {
        ...
        return SceneIndexManager.queryNeighbors(getScriptManager().getBlocksIndex(),
                player.getPosition().toXZDoubleArray(), gingerps.getConfig().server.game.loadEntitiesForPlayerRange);
    }
    ...
    public List<SceneGroup> playerMeetGroups(Player player, SceneBlock block) {
        List<SceneGroup> sceneGroups = SceneIndexManager.queryNeighbors(block.sceneGroupIndex, player.getPosition().toDoubleArray(),
                gingerps.getConfig().server.game.loadEntitiesForPlayerRange);
    ...
    public void onLoadBlock(SceneBlock block, List<Player> players) {
        ...
        onLoadGroup(groups);
        gingerps.getLogger().info("Scene {} Block {} loaded.", this.getId(), block.id);
    }
    ...
    public void onLoadGroup(List<SceneGroup> groups) {
        ...
        gingerps.getLogger().info("Scene {} loaded {} group(s)", this.getId(), groups.size());
    }
    ...
    public void onUnloadBlock(SceneBlock block) {
        ...
        gingerps.getLogger().info("Scene {} Block {} is unloaded.", this.getId(), block.id);
    }
    // Gadgets
    ...
    private List<SceneNpcBornEntry> loadNpcForPlayer(Player player) {
        ...
        if (sceneNpcBornEntries.size() > 0) {
            ...
            gingerps.getLogger().debug("Loaded Npc Group Suite {}", sceneNpcBornEntries);
        }
        return npcList;
    }
