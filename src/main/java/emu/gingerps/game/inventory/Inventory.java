...
import emu.gingerps.server.packet.send.PacketAvatarEquipChangeNotify;
import emu.gingerps.server.packet.send.PacketItemAddHintNotify;
import emu.gingerps.server.packet.send.PacketStoreItemChangeNotify;
import emu.gingerps.server.packet.send.PacketStoreItemDelNotify;
...
import static emu.gingerps.config.Configuration.INVENTORY_LIMITS;
            ...
            default:
                switch (item.getItemData().getMaterialType()) {
                    ...
                    case MATERIAL_NAMECARD:
                        gingerps.getLogger().warn("Attempted to add a "+item.getItemData().getMaterialType().name()+" to inventory, but item definition lacks isUseOnGain. This indicates a Resources error.");
                        return null;
                    default:
                        if (tab == null) {
                            return null;
