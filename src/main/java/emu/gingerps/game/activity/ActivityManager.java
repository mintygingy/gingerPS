...
import emu.gingerps.server.packet.send.PacketActivityScheduleInfoNotify;
        ...
        try {
            DataLoader.loadList("ActivityConfig.json", ActivityConfigItem.class).forEach(item -> {
                ...
                if (activityData == null) {
                    gingerps.getLogger().warn("activity {} not exist.", item.getActivityId());
                    return;
                }
            ...
            gingerps.getLogger().info("Enable {} activities.", activityConfigItemMap.size());
        } catch (Exception e) {
            gingerps.getLogger().error("Unable to load activities config.", e);
        }
