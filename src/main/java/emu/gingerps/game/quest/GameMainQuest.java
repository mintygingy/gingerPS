...
import emu.gingerps.data.binout.ScriptSceneData;
import emu.gingerps.data.excels.QuestData;
import emu.gingerps.game.quest.enums.LogicType;
import emu.gingerps.game.quest.enums.QuestTrigger;
import emu.gingerps.scripts.ScriptLoader;
import emu.gingerps.server.packet.send.PacketCodexDataUpdateNotify;
...
import emu.gingerps.net.proto.ChildQuestOuterClass.ChildQuest;
import emu.gingerps.net.proto.ParentQuestOuterClass.ParentQuest;
import emu.gingerps.net.proto.QuestOuterClass.Quest;
import emu.gingerps.server.packet.send.PacketFinishedParentQuestUpdateNotify;
import emu.gingerps.server.packet.send.PacketQuestListUpdateNotify;
import emu.gingerps.server.packet.send.PacketQuestProgressUpdateNotify;
...
public class GameMainQuest {
    ...
    public void setQuestVar(int i, int value) {
        ...
        gingerps.getLogger().debug("questVar {} value changed from {} to {}", i, previousValue, value);
    }

    public void incQuestVar(int i, int inc) {
        int previousValue = this.questVars[i];
        this.questVars[i] += inc;
        gingerps.getLogger().debug("questVar {} value incremented from {} to {}", i, previousValue, previousValue + inc);
    }

    public void decQuestVar(int i, int dec) {
        int previousValue = this.questVars[i];
        this.questVars[i] -= dec;
        gingerps.getLogger().debug("questVar {} value decremented from {} to {}", i, previousValue, previousValue - dec);
    }
    ...
    public void addRewindPoints() {
        ...
        if (cs == null) {
            gingerps.getLogger().debug("Couldn't find " + script);
            return;
        }
        ...
        try {
                                ...
                                if (dummyPointScript != null) {
                                    ...
                                    if (dummyPointMap != null) {
                                        ...
                                        if (avatarPosPos != null) {
                                            ...
                                            gingerps.getLogger().debug("Succesfully loaded rewind position for subQuest {}",subId);
                                        }
                                    }
                                }
                            }
            ...
            gingerps.getLogger().error("An error occurred while loading rewind positions");
        }
    }
    ...
    public void tryAcceptSubQuests(QuestTrigger condType, String paramStr, int... params) {
        try {
            ...
            gingerps.getLogger().error("An error occurred while trying to accept quest.", e);
        }
    ...
    public void tryFailSubQuests(QuestTrigger condType, String paramStr, int... params) {
        try {
            ...
            gingerps.getLogger().error("An error occurred while trying to fail quest.", e);
        }
    }
    ...
    public void tryFinishSubQuests(QuestTrigger condType, String paramStr, int... params) {
        try {
            ...
            gingerps.getLogger().debug("An error occurred while trying to finish quest.", e);
        }
    }
