...
import emu.gingerps.data.excels.HomeWorldLevelData;
...
public class GameHome {
    ...
    public HomeSceneItem getHomeSceneItem(int sceneId) {
        return sceneMap.computeIfAbsent(sceneId, e -> {
            ...
            if (defaultItem != null) {
                gingerps.getLogger().info("Set player {} home {} to initial setting", ownerUid, sceneId);
    ...
    public Player getPlayer() {
        if (this.player == null)
            this.player = gingerps.getGameServer().getPlayerByUid((int) this.ownerUid, true);
        return this.player;
    }
