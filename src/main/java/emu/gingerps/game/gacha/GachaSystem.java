package emu.gingerps.game.gacha;
...
import emu.gingerps.game.gacha.GachaBanner.BannerType;
...
import emu.gingerps.net.proto.GachaItemOuterClass.GachaItem;
import emu.gingerps.net.proto.GachaTransferItemOuterClass.GachaTransferItem;
...
import emu.gingerps.server.game.GameServerTickEvent;
import emu.gingerps.server.packet.send.PacketDoGachaRsp;
            ...
            if (banners.size() > 0) {
                for (GachaBanner banner : banners) {
                    ...
                    if (banner.isDeprecated()) {
                        gingerps.getLogger().error("A Banner has not been loaded because it contains one or more deprecated fields. Remove the fields mentioned above and reload.");
                    } else if (banner.isDisabled()) {
                        gingerps.getLogger().debug("A Banner has not been loaded because it is disabled.");
                    } else {
                ...
                gingerps.getLogger().debug("Banners successfully loaded.");
            } else {
                gingerps.getLogger().error("Unable to load banners. Banners size is 0.");
            }
        } catch (Exception e) {
    ...
    private synchronized void startWatcher(GameServer server) {
        if (this.watchService == null) {
            try {
                ...
                gingerps.getLogger().error("Unable to load the Gacha Manager Watch Service. If ServerOptions.watchGacha is true it will not auto-reload");
                e.printStackTrace();
            }
        } else {
            gingerps.getLogger().error("Cannot reinitialise watcher ");
        }
    }
            ...
            try {
                ...
                for (WatchEvent<?> event : watchKey.pollEvents()) {
                    ...
                    if (changed.endsWith("Banners.json")) {
                        gingerps.getLogger().info("Change detected with banners.json. Reloading gacha config");
                        this.load();
                    }
                }
                ...
                if (!valid) {
                    gingerps.getLogger().error("Unable to reset Gacha Manager Watch Key. Auto-reload of banners.json will no longer work.");
                    return;
                }
            } catch (Exception e) {
                e.printStackTrace();
