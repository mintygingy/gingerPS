...
import emu.gingerps.net.proto.GachaInfoOuterClass.GachaInfo;
import emu.gingerps.net.proto.GachaUpInfoOuterClass.GachaUpInfo;
...
public class GachaBanner {
    ...
    private void warnDeprecated(String name, String replacement) {
        gingerps.getLogger().error("Deprecated field found in Banners config: "+name+" was replaced back in early May 2022, use "+replacement+" instead. You MUST remove this field from your config.");
    ...
    public void onLoad() {
        ...
        if (this.previewPrefabPath != null && this.previewPrefabPath.equals("UI_Tab_" + this.prefabPath))
            gingerps.getLogger().error("Redundant field found in Banners config: previewPrefabPath does not need to be specified if it is identical to prefabPath prefixed with \"UI_Tab_\".");
    ...
    public GachaInfo toProto(Player player) {
        ...
        // gingerps.getLogger().info("record = " + record);
