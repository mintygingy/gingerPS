package emu.gingerps.game.managers.blossom;
...
import emu.gingerps.data.excels.RewardPreviewData;
...
import emu.gingerps.server.packet.send.PacketBlossomBriefInfoNotify;
...
public class BlossomManager {
    ...
    public void initBlossom(EntityGadget gadget) {
        ...
        gadgetWorktop.setOnSelectWorktopOptionEvent((GadgetWorktop context, int option) -> {
            ...
            synchronized (blossomActivities) {
                ...
                gingerps.getLogger().info("Blossom Monsters:"+monsters);
    ...
    private static Integer getPreviewReward(BlossomType type, int worldLevel) {
        ...
        if (type == null) {
            gingerps.getLogger().error("Illegal blossom type {}",type);
            return null;
        }
        ...
        for (var data : dataMap.values()) {
            if (blossomChestId == data.getBlossomChestId()) {
                ...
                if (worldLevel > dropVecList.length) {
                    gingerps.getLogger().error("Illegal world level {}", worldLevel);
                    return null;
                }
                return dropVecList[worldLevel].getPreviewReward();
            }
        }
        gingerps.getLogger().error("Cannot find blossom type {}",type);
        return null;
    }
                ...
                if (activeChest.getChest() == chest) {
                    ...
                    if (pay) {
                        ...
                        if (blossomRewards == null) {
                            gingerps.getLogger().error("Blossom could not support world level : "+worldLevel);
                            return null;
                        }
