package emu.gingerps.game.managers.energy;
...
import emu.gingerps.data.excels.MonsterData.HpDrops;
...
import emu.gingerps.game.inventory.MaterialType;
...
import emu.gingerps.game.props.MonsterType;
import emu.gingerps.game.props.WeaponType;
import emu.gingerps.game.props.ItemUseAction.ItemUseAction;
import emu.gingerps.game.props.ItemUseAction.ItemUseAddEnergy;
import emu.gingerps.net.proto.AbilityActionGenerateElemBallOuterClass.AbilityActionGenerateElemBall;
import emu.gingerps.net.proto.AbilityIdentifierOuterClass.AbilityIdentifier;
...
import emu.gingerps.net.proto.EvtBeingHitInfoOuterClass.EvtBeingHitInfo;
...
public class EnergyManager extends BasePlayerManager {
    ...
    public static void initialize() {
        ...
        try {
            ...
            gingerps.getLogger().debug("Energy drop data successfully loaded.");
        }
        catch (Exception ex) {
            gingerps.getLogger().error("Unable to load energy drop data.", ex);
        }
        ...
        try {
            ...
            gingerps.getLogger().debug("Skill particle generation data successfully loaded.");
        }
        catch (Exception ex) {
            gingerps.getLogger().error("Unable to load skill particle generation data data.", ex);
        }
    }
    ...
    private int getBallCountForAvatar(int avatarId) {
        ...
        if (!skillParticleGenerationData.containsKey(avatarId)) {
            gingerps.getLogger().warn("No particle generation data for avatarId {} found.", avatarId);
        }
    ...
    private void generateElemBallDrops(EntityMonster monster, int dropId) {
        ...
        if (!energyDropData.containsKey(dropId)) {
            gingerps.getLogger().warn("No drop data for dropId {} found.", dropId);
            return;
        }
