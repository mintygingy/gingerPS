...
public final class CommandMap {
    ...
    public CommandMap registerCommand(String label, CommandHandler command) {
        gingerps.getLogger().debug("Registered command: " + label);
    ...
    public CommandMap unregisterCommand(String label) {
        gingerps.getLogger().debug("Unregistered command: " + label);
    ...
    private Player getTargetPlayer(String playerId, Player player, Player targetPlayer, List<String> args) {
        ...
        for (int i = 0; i < args.size(); i++) {
            ...
            if (arg.startsWith("@")) {
                ...
                targetPlayer = gingerps.getGameServer().getPlayerByUid(uid, true);
        ...
        if (targetPlayerIds.containsKey(playerId)) {
            targetPlayer = gingerps.getGameServer().getPlayerByUid(targetPlayerIds.getInt(playerId), true);
    ...
    private boolean setPlayerTarget(String playerId, Player player, String targetUid) {
        ...
        Player targetPlayer = gingerps.getGameServer().getPlayerByUid(uid, true);
        if (targetPlayer == null) {
            CommandHandler.sendTranslatedMessage(player, "commands.execution.player_exist_error");
            return false;
        }
    ...
    public void invoke(Player player, Player targetPlayer, String rawMessage) {
        ...
        if (SERVER.logCommands) {
            if (player != null) {
                gingerps.getLogger().info("Command used by [" + player.getAccount().getUsername() + " (Player UID: " + player.getUid() + ")]: " + rawMessage);
            } else {
                gingerps.getLogger().info("Command used by server console: " + rawMessage);
            }
        }
        ...
        if (!gingerps.getPermissionHandler().checkPermission(player, targetPlayer, annotation.permission(), this.annotations.get(label).permissionTargeted())) {
            return;
        }
     ...
     */
    private void scan() {
        Reflections reflector = gingerps.reflector;
        ...
        classes.forEach(annotated -> {
            try {
                ...
                else gingerps.getLogger().error("Class " + annotated.getName() + " is not a CommandHandler!");
            } catch (Exception exception) {
                gingerps.getLogger().error("Failed to register command handler for " + annotated.getSimpleName(), exception);
            }
        });
    }
}
