                    ...
                    case "finish" -> {
                        if (mailBuilder.constructionStage == 3) {
                            if (!mailBuilder.sendToAll) {
                                gingerps.getGameServer().getPlayerByUid(mailBuilder.recipient, true).sendMail(mailBuilder.mail);
                                ...
                                DatabaseHelper.getByGameClass(Player.class).forEach(player -> {
                                    var onlinePlayer = gingerps.getGameServer().getPlayerByUid(player.getUid(), false);
    ...
    public static class MailBuilder {
        ...
        public MailBuilder(boolean sendToAll, Mail mail) {
            if (sendToAll) {
                ...
                gingerps.getLogger().error("Please use MailBuilder(int, mail) when not sending to all");
                Thread.dumpStack();
            }
        }
    }
