...
import emu.gingerps.scripts.data.*;
import emu.gingerps.scripts.service.ScriptMonsterSpawnService;
import emu.gingerps.scripts.service.ScriptMonsterTideService;
...
public class SceneScriptManager {
    ...
    public SceneScriptManager(Scene scene) {
        ...
        // TEMPORARY
        if (this.getScene().getId() < 10 && !gingerps.getConfig().server.game.enableScriptInBigWorld) {
            return;
        }
    ...
    public void registerTrigger(SceneTrigger trigger) {
        ...
        gingerps.getLogger().debug("Registered trigger {}", trigger.name);
    }
    ...
    public void registerRegion(EntityRegion region) {
        ...
        gingerps.getLogger().debug("Registered region {} from group {}", region.getMetaRegion().config_id, region.getGroupId());
    }
    ...
    public void checkRegions() {
        ...
        for (var region : this.regions.values()) {
            ...
            if (region.hasNewEntities()) {
                gingerps.getLogger().trace("Call EVENT_ENTER_REGION_{}",region.getMetaRegion().config_id);
        ...
        try {
            ...
            for (SceneTrigger trigger : relevantTriggers) {
                try {
                    ...
                    gingerps.getLogger().trace("Call Condition Trigger {}, [{},{},{}]", trigger.condition, params.param1, params.source_eid, params.target_eid);
                    if (ret.isboolean() && ret.checkboolean()) {
                        // the SetGroupVariableValueByGroup in tower need the param to record the first stage time
                        this.callScriptFunc(trigger.action, trigger.currentGroup, params);
                        gingerps.getLogger().trace("Call Action Trigger {}", trigger.action);
                        ...
                        gingerps.getLogger().debug("Condition Trigger {} returned {}", trigger.condition, ret);
                    }
