...
import emu.gingerps.auth.AuthenticationSystem.AuthenticationRequest;
...
public final class DefaultAuthenticators {
    ...
    public static class PasswordAuthenticator implements Authenticator<LoginResultJson> {
        ...
        public LoginResultJson authenticate(AuthenticationRequest request) {
            ...
            int playerCount = gingerps.getGameServer().getPlayers().size();
            ...
            if (ACCOUNT.maxPlayer <= -1 || playerCount < ACCOUNT.maxPlayer) {
                ...
                if (account == null && ACCOUNT.autoCreate) {
                    ...
                    if (account == null) {
                        ...
                        gingerps.getLogger().info(translate("messages.dispatch.account.account_login_create_error", address));
                    } else {
                        // Continue with login.
                        successfulLogin = true;

                        // Log the creation.
                        gingerps.getLogger().info(translate("messages.dispatch.account.account_login_create_success", address, response.data.account.uid));
                    }
            ...
            gingerps.getLogger().info(loggerMessage);
    ...
    public static class TokenAuthenticator implements Authenticator<LoginResultJson> {
        ...
        public LoginResultJson authenticate(AuthenticationRequest request) {
            ...
            // Log the attempt.
            gingerps.getLogger().info(translate("messages.dispatch.account.login_token_attempt", address));
