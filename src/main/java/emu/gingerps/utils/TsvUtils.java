...
import static emu.gingerps.utils.Utils.nonRegexSplit;
...
public class TsvUtils {
            ...
            try {
                ...
                this.children.forEach((name, tree) -> {
                    ...
                    try {
                        ...
                        gingerps.getLogger().error("Exception while setting field "+name+" ("+field.classType+")"+" for class "+objClass+" - ",e);
                    }
                });
                return obj;
            } catch (Exception e) {
                // System.out.println("Exception while creating object of class "+objClass+" - "+e);
                gingerps.getLogger().error("Exception while creating object of class "+objClass+" - ",e);
                return null;
            }
        }
        ...
        try (val fileReader = Files.newBufferedReader(filename, StandardCharsets.UTF_8)) {
            ...
            // return fileReader.lines().map(line -> {
                ...
                try {
                    ...
                    gingerps.getLogger().warn("Error deserializing an instance of class "+classType.getCanonicalName());
                    gingerps.getLogger().warn("At token #"+t+" of #"+m);
                    gingerps.getLogger().warn("Header names are: "+headerNames.toString());
                    gingerps.getLogger().warn("Tokens are: "+tokens.toString());
                    gingerps.getLogger().warn("Stacktrace is: ", e);
            ...
            gingerps.getLogger().error("Error loading file '"+filename+"' - Stacktrace is: ", e);
            return null;
        }
    }
    ...
    public static <T> List<T> loadTsjToListSetField(Path filename, Class<T> classType) {
        try (val fileReader = Files.newBufferedReader(filename, StandardCharsets.UTF_8)) {
            ...
            gingerps.getLogger().error("Error loading file '"+filename+"' - File does not exist. You are missing resources. Note that this file may exist in JSON, TSV, or TSJ format, any of which are suitable.");
            return null;
        } catch (IOException e) {
            gingerps.getLogger().error("Error loading file '"+filename+"' - Stacktrace is: ", e);
            return null;
        } catch (NoSuchMethodException e) {
            gingerps.getLogger().error("Error loading file '"+filename+"' - Class is missing NoArgsConstructor");
            return null;
        }
    }
    ...
    public static <T> List<List<T>> loadTsjsToListsConstructor(Class<T> classType, Path... filenames) throws Exception {
        ...
        if (pair == null) {
            gingerps.getLogger().error("No AllArgsContructor found for class: "+classType);
            return null;
        }
            ...
            try (val fileReader = Files.newBufferedReader(filename, StandardCharsets.UTF_8)) {
                ...
                return fileReader.lines().parallel().map(line -> {
                    ...
                    try {
                        ...
                        gingerps.getLogger().warn("Error deserializing an instance of class "+classType.getCanonicalName()+" : "+constructor.getName());
                        gingerps.getLogger().warn("At token #"+t+" of #"+m);
                        gingerps.getLogger().warn("Arg names are: "+Arrays.toString(conArgNames));
                        gingerps.getLogger().warn("Arg types are: "+Arrays.toString(argTypes));
                        gingerps.getLogger().warn("Default Args are: "+Arrays.toString(defaultArgs));
                        gingerps.getLogger().warn("Args are: "+Arrays.toString(args));
                        gingerps.getLogger().warn("Header names are: "+headerNames.toString());
                        gingerps.getLogger().warn("Header types are: "+IntStream.of(argPositions).mapToObj(i -> (i >= 0) ? argTypes[i] : null).toList());
                        gingerps.getLogger().warn("Tokens are: "+tokens.toString());
                        gingerps.getLogger().warn("Stacktrace is: ", e);
                        return null;
                    }
