...
public final class Utils {
    ...
    public static boolean copyFromResources(String resource, String destination) {
        try (InputStream stream = gingerps.class.getResourceAsStream(resource)) {
            if (stream == null) {
                gingerps.getLogger().warn("Could not find resource: " + resource);
                return false;
            }
            ...
            gingerps.getLogger().warn("Unable to copy resource " + resource + " to " + destination, exception);
            return false;
        }
    }
    ...
    public static void logObject(Object object) {
        gingerps.getLogger().info(JsonUtils.encode(object));
    }
    ...
    public static void startupCheck() {
        ConfigContainer config = gingerps.getConfig();
        Logger logger = gingerps.getLogger();
        boolean exit = false;
    ...
    public static String readFromInputStream(@Nullable InputStream stream) {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(stream, StandardCharsets.UTF_8))) {
            ...
            gingerps.getLogger().warn("Failed to read from input stream.");
    ...
    public static int lerp(int x, int[][] xyArray) {
        try {
            ...
            gingerps.getLogger().error("Malformed lerp point array. Must be of form [[x0, y0], ..., [xN, yN]].");
        }
        return 0;
    }
