...
public final class FileUtils {
    ...
    private static final Path DATA_USER_PATH = Path.of(gingerps.config.folderStructure.data);
    private static final Path PACKETS_PATH = Path.of(gingerps.config.folderStructure.packets);
    private static final Path PLUGINS_PATH = Path.of(gingerps.config.folderStructure.plugins);
    ...
    static {
        ...
        try {
            var uri = gingerps.class.getResource("/defaults/data").toURI();
            switch (uri.getScheme()) {
                ...
                case "file":  // When running in an IDE
                    ...
                    break;
                default:
                gingerps.getLogger().error("Invalid URI scheme for class resources: "+uri.getScheme());
                    break;
            }
        } catch (URISyntaxException | IOException e) {
            // Failed to load this jar. How?
            gingerps.getLogger().error("Failed to load jar?!");
        } finally {
            DATA_DEFAULT_PATH = path;
            gingerps.getLogger().debug("Setting path for default data: "+path.toAbsolutePath());
        }

        // Setup Resources path
        final String resources = gingerps.config.folderStructure.resources;
        fs = null;
        ...
        if (resources.endsWith(".zip")) {  // Would be nice to support .tar.gz too at some point, but it doesn't come for free in Java
            try {
                ...
                gingerps.getLogger().error("Failed to load resources zip \"" + resources + "\"");
            }
        }
        ...
        if (fs != null) {
            ...
            try (Stream<Path> pathStream = Files.find(root, 3, (p, a) -> {
                ...
                if (excelBinOutput.isPresent()) {
                    ...
                    if (path == null)
                        path = root;
                    gingerps.getLogger().debug("Resources will be loaded from \"" + resources + "/" + path.toString() + "\"");
                } else {
                    gingerps.getLogger().error("Failed to find ExcelBinOutput in resources zip \"" + resources + "\"");
                }
            } catch (IOException e) {
                gingerps.getLogger().error("Failed to scan resources zip \"" + resources + "\"");
            }
        }
        RESOURCES_PATH = path;

        // Setup Scripts path
        final String scripts = gingerps.config.folderStructure.scripts;
    ...
    public static void write(String dest, byte[] bytes) {
        ...
        try {
            ...
            gingerps.getLogger().warn("Failed to write file: " + dest);
        }
    }
    ...
    public static byte[] read(Path path) {
        try {
            ...
            gingerps.getLogger().warn("Failed to read file: " + path);
        }
    ...
    public static InputStream readResourceAsStream(String resourcePath) {
        return gingerps.class.getResourceAsStream(resourcePath);
    }

    public static byte[] readResource(String resourcePath) {
        try (InputStream is = gingerps.class.getResourceAsStream(resourcePath)) {
            return is.readAllBytes();
        } catch (Exception exception) {
            gingerps.getLogger().warn("Failed to read resource: " + resourcePath);
    ...
    public static void copyResource(String resourcePath, String destination) {
        try {
            ...
            gingerps.getLogger().warn("Failed to copy resource: " + resourcePath + "\n" + exception);
        }
    }
    ...
    public static List<Path> getPathsFromResource(String folder) throws URISyntaxException {
        try {
            // file walks JAR
            return Files.walk(Path.of(gingerps.class.getResource(folder).toURI()))
