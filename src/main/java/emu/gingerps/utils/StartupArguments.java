...
import emu.gingerps.BuildConfig;
...
public final class StartupArguments {
    ...
    private static final Map<String, Function<String, Boolean>> argumentHandlers = Map.of(
        ...
        "-lang", parameter -> {
            gingerps.setPreferredLanguage(parameter); return false;
        },"-game", parameter -> {
            gingerps.setRunModeOverride(ServerRunMode.GAME_ONLY); return false;
        },"-dispatch", parameter -> {
            gingerps.setRunModeOverride(ServerRunMode.DISPATCH_ONLY); return false;
        },
    ...
    private static boolean printVersion(String parameter) {
        System.out.println("gingerps version: " + BuildConfig.VERSION + "-" + BuildConfig.GIT_HASH); return true;
    }
    ...
    private static boolean enableDebug(String parameter) {
        ...
        gingerps.getLogger().setLevel(DEBUG_MODE_INFO.serverLoggerLevel);
        gingerps.getLogger().debug("The logger is now running in debug mode.");
