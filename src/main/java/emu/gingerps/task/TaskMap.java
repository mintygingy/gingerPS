...
public final class TaskMap {
    ...
    public static TaskMap getInstance() {
        return gingerps.getGameServer().getTaskMap();
    }
        ...
        classes.forEach(annotated -> {
            try {
                ...
                if (object instanceof TaskHandler) {
                    ...
                    gingerps.getLogger().error("Class " + annotated.getName() + " is not a TaskHandler!");
                }
            } catch (Exception exception) {
                gingerps.getLogger().error("Failed to register task handler for " + annotated.getSimpleName(), exception);
            }
        });
        try {
            Scheduler scheduler = schedulerFactory.getScheduler();
